<script lang="ts">
	...
	onMount(async () => {
		...
		try {
			...
			error = e instanceof Error ? e.message : 'Pesan terkirim belum bisa dimuat.';
		} finally {
			loading = false;
		}
	});
...
<svelte:head><title>Terkirim · Wingback</title></svelte:head>

<main class="list-page">
	<header class="list-header">
		<div>
			<p class="eyebrow">JEJAK PESANMU</p>
			<h1>Terkirim</h1>
			<p class="intro">Lihat semua surat yang pernah kamu lepaskan ke perjalanan.</p>
		</div>
	...
	{:else if list.length === 0}<div class="empty-state">
			<span class="empty-glyph">⌁</span>
			<h2>Belum ada yang dilepaskan.</h2>
			<p>Tulis surat pertama dan biarkan ia berjalan.</p>
		</div>
			...
			{#each list as m (m.id)}
				<a class="message-card" href={`/track/${m.id}`}>
					...
					<div class="message-main">
						...
						<h2>Perjalanan sejauh {formatDistance(m.distance_km)}</h2>
						<p>{m.body}</p>
					</div>

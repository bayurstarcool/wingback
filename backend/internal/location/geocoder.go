...
type Resolver interface {
	ResolveCity(ctx context.Context, lat, lng float64) (string, error)
}
...
func (r *NominatimResolver) ResolveCity(ctx context.Context, lat, lng float64) (string, error) {
	if lat < -90 || lat > 90 || lng < -180 || lng > 180 {
		return "", fmt.Errorf("coordinate out of range")
	}
	endpoint, err := url.Parse(r.BaseURL)
	if err != nil {
		return "", fmt.Errorf("parse geocoder URL: %w", err)
	}
	...
	if err != nil {
		return "", fmt.Errorf("create geocoder request: %w", err)
	}
	...
	if res.StatusCode != http.StatusOK {
		return "", fmt.Errorf("geocoder returned HTTP %d", res.StatusCode)
	}

	var payload nominatimResponse
	if err := json.NewDecoder(res.Body).Decode(&payload); err != nil {
		return "", fmt.Errorf("decode geocoder response: %w", err)
	}
	...
	return "", fmt.Errorf("geocoder returned no city")
}
